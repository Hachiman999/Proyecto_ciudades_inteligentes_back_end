...
router.get(RutaGET_DatosNodo, (req, res) => {
    ...
    Connection.query(sentencia, [NumNodo], (err, result) => { //Hace la consulta, ejecuta un función que puede retornar un error (err) o un resultado (result)
        if (result != "") {
            console.log(result);
            res.json(result);
            res.status(200);
...
router.get(RutaGET_EntradaDatos, (req, res) => {
    ...
    Connection.query(sentencia, (err, result) => { //Hace la consulta, ejecuta un función que puede retornar un error (err) o un resultado (result)
        if (result != "") {
            console.log(result);
            res.json(result);
            res.status(200);
...
router.get(RutaGET_Promedio_varibles_nodos, (req, res) => {
    var sentencia = 'SELECT AVG(DAM.Temperatura) AS PROM_TA, AVG(DAM.Humedad) AS PROM_H, AVG(DAM.Vel_viento) AS PROM_VV, AVG(DAG.Temperatura_agua) AS PROM_TAG, AVG(DAG.Nivel_agua) AS PROM_NA, AVG(DAG.Caudal) AS PROM_C, AVG(DAG.Flujo) AS PROM_F FROM datosnodo DD JOIN datos_arduino_ambiente DAM ON DD.NumNodo = DAM.ID_Arduino JOIN datos_arduino_agua DAG ON DAG.ID_Arduino = DAM.ID_Arduino';
    Connection.query(sentencia, (err, result) => { //Hace la consulta, ejecuta un función que puede retornar un error (err) o un resultado (result)
        if (result != "") {
            console.log(result);
            res.json(result);
...
router.post(RutaPOST, (req, res) => {
    router.get('https://api.openweathermap.org/data/2.5/onecall?lat=3.44&lon=-76.52&units=metric&exclude=hourly,daily&appid=c0f5717aa0d453fc7c3843390e59e72e', (req, res) => {
        console.log(res);
    });
    json1 = req.body;
    ...
    console.log(NumNodo, Temp_Amb, Hum, Vel, Dir, Temp_Ag, Niv, Cau, Flu, Pun_rocio, Pres, Nubes, Fecha, Hora);

    var sentencia1 = 'INSERT INTO datos_arduino_ambiente (ID_Arduino, Temperatura, Humedad, Vel_viento, Dir_Viento, Fecha, Hora) VALUES (?,?,?,?,?,?,?)'
    var sentencia2 = 'INSERT INTO datos_arduino_agua (ID_Arduino, Temperatura_agua, Nivel_agua, Caudal, Flujo, Fecha, Hora) VALUES (?,?,?,?,?,?,?)';
    var sentencia3 = 'INSERT INTO datos_api (ID_Arduino, Punto_rocio, Presion, Nubosidad, Fecha, Hora) VALUES (?,?,?,?,?,?)';
    if (NumNodo && Temp_Amb && Hum && Vel && Dir && Temp_Ag && Niv && Cau && Flu && Pun_rocio && Pres && Nubes && Fecha && Hora) {
        Connection.query(sentencia1, [NumNodo, Temp_Amb, Hum, Vel, Dir, Fecha, Hora], (err1, result1) => {
            res.status(201);
            //console.log(result1);
            //console.log('RowAffected datos_arduino_ambiente: '+ result1.affectedRows);
        });
        Connection.query(sentencia2, [NumNodo, Temp_Ag, Niv, Cau, Flu, Fecha, Hora], (err2, result2) => {
            res.status(201);
            //console.log(result2);
            //console.log('RowAffected datos_arduino_agua: ' + result2.affectedRows);
        });
        Connection.query(sentencia3, [NumNodo, Pun_rocio, Pres, Nubes, Fecha, Hora], (err3, result3) => {
            res.status(201);
            //console.log(result2);
            //console.log('RowAffected datos_arduino_agua: ' + result2.affectedRows);
        });
...
router.put(RutaPUT_Cofig_nodo, (req, res) => {
    ...
    var sentencia = 'UPDATE datosnodo SET NumNodo=?, Longitud=?, Latitud=?, Estado=? WHERE ID_Nodo=?';
...
router.put(RutaPUT_User, (req, res) => {
    ...
    var sentencia = 'UPDATE datos_user SET Tipo_ID=?, ID=?, Nombre=?, Apellido=?, `Celular`=?, Dirección=? WHERE Mail=?';
...
router.post(RutaPOST_nuevo_nodo, (req, res) => {
    ...
    var sentencia = 'INSERT INTO datosnodo (ID_Nodo, NumNodo, Longitud, Latitud, Estado, Bateria) VALUES (null,?,?,?,?,?)'
